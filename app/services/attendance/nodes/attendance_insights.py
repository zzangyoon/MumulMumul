...
system_prompt = """
    ...
    - JSON 형식으로 출력한다.
    [출력 내용 설명]
    - attendance_insights: 학생별 출결 인사이트 리스트
        - user_id: 학생 고유 ID
        - risk_level: 학생의 출결 위험 수준
            - 고위험: 출결 상태가 매우 나쁘고 즉각적인 조치가 필요함
            - 위험: 출결 상태가 나쁘고 주의 깊은 모니터링이 필요함
            - 주의: 출결 상태에 약간의 문제가 있으나 심각하지 않음
            - 정상: 출결 상태가 양호함
            - 미확인: 출결 데이터를 기반으로 판단할 수 없음
        - pattern_type: 학생의 주요 출결 패턴 유형
            - 안정형: 출결이 전반적으로 안정적임
            - 지각형: 지각이 잦음
            - 조퇴형: 조퇴가 잦음
            - 결석형: 결석이 잦음
            - 불규칙형: 출결이 불규칙함
            - 공백위험형: 장기간 출결 공백이 있음
            - 신규: 최근에 등록한 학생으로 출결 데이터가 부족함
            - 데이터없음: 출결 데이터가 전혀 없음
        - trend: 최근 출결 추세 변화 값
            - 양수는 개선(+), 음수는 악화(-), 0에 가까울수록 변화 없음
        - ops_action: 운영진이 취해야 할 권장 조치와 성향 정보를 반영한 커뮤니케이션 가이드 
