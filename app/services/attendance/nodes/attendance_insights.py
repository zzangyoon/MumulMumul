...
system_prompt = """
    ...
    [출력 형식]
    - JSON 형식으로 출력한다.
    - 반드시 다음 Pydantic 모델 AttendanceInsightOutput 스키마를 준수한다.
    {format_instructions}
    """
...
def generate_attendance_insights(
    ...
    llm = openai_chat_model("gpt-4.0-mini", temperature=0.2)

    llm_chain = prompt | llm | parser

    # 배치 사이즈 10명 학생별로 LLM 호출
    batch_size = 10
    for i in range(0, len(report.students), batch_size):
        batch_students = report.students[i : i + batch_size]
        # 배치 학생들을 위한 context 생성

        formatted_prompt = prompt.format_prompt(attendance_data=[
            f"""
            [학생 ID: {student.student_id}, 이름: {student.name}]
        ...
        insights = llm_chain.invoke(formatted_prompt)

        # 배치 학생 통계에 인사이트 반영
        for student_stat, insight in zip(batch_students, insights):
            student_stat.risk_level = insight.risk_level
            student_stat.pattern_type = insight.pattern_type
            student_stat.trend = insight.trend
            student_stat.ops_action = insight.ops_action

    # summary의 고위험/위험 학생 수 갱신
    high_risk_count = sum(1 for s in report.students if s.risk_level == "고위험")
    warning_count = sum(1 for s in report.students if s.risk_level == "위험")
